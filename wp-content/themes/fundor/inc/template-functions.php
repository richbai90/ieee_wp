 ...
 */
if ( ! function_exists( 'fundor_is_frontpage' ) ) {

	function fundor_is_frontpage() {
		return ( is_front_page() || is_home() );
	}
}


if (!function_exists('fundor_is_elementor_activated')) {
    function fundor_is_elementor_activated() {
        return function_exists('elementor_load_plugin_textdomain');
    }
}

if ( ! function_exists( 'fundor_page_enable_breadcrumb' ) ) {
	/**
	 * @return bool
	 */
	function fundor_page_enable_breadcrumb() {

		if ( ! is_page() ) {
			return true;
		}

		$check = fundor_get_metabox( get_the_ID(), 'osf_enable_breadcrumb', true );
...
if ( ! function_exists( 'fundor_get_query' ) ) {
	 ...
	 * @return WP_Query
	 */
	function fundor_get_query( $args ) {
		global $wp_query;
		$default  = array(
			'post_type' => 'post',
		);
...
if ( ! function_exists( 'fundor_get_placeholder_image' ) ) {

	/**
	 * @return string
	 */
	function fundor_get_placeholder_image() {
		return get_parent_theme_file_uri( '/assets/images/placeholder.png' );
	}

}

if ( ! function_exists( 'fundor_is_osf_framework_activated' ) ) {
	/**
	 * Query WooCommerce activation
	 */
	function fundor_is_osf_framework_activated() {
		return class_exists( 'FundorCore' ) ? true : false;
	}
}


if ( ! function_exists( 'fundor_get_metabox' ) ) {
	 ...
	 * @return bool|mixed
	 */
	function fundor_get_metabox( $id, $key, $default = false ) {
...
if ( ! function_exists( 'fundor_is_blog_archive' ) ) {
	function fundor_is_blog_archive() {
		return ( is_home() && is_front_page() ) || is_category() || is_tag() || is_post_type_archive( 'post' );
	}
}

if ( ! function_exists( 'fundor_get_excerpt' ) ) {
	function fundor_get_excerpt( $excerpt_length = 55 ) {
		global $post;
...
if ( ! function_exists( 'fundor_do_shortcode' ) ) {
	 ...
	 */
	function fundor_do_shortcode( $tag, array $atts = array(), $content = null ) {
...
if ( ! function_exists( 'fundor_nav_menu_social_icons' ) ) {
	 ...
	 */
	function fundor_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
		// Get supported social icons.
		$social_icons = fundor_social_links_icons();
...
add_filter( 'walker_nav_menu_start_el', 'fundor_nav_menu_social_icons', 10, 4 );

if ( ! function_exists( 'fundor_social_links_icons' ) ) {
	 ...
	 */
	function fundor_social_links_icons() {
		...
		return apply_filters( 'fundor_social_links_icons', $social_links_icons );
	}
}

if ( ! function_exists( 'fundor_dropdown_icon_to_menu_link' ) ) {
	/**
	 ...
	 */
	function fundor_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
...
add_filter( 'nav_menu_item_title', 'fundor_dropdown_icon_to_menu_link', 10, 4 );

if ( ! function_exists( 'fundor_is_header_builder' ) ) {
	/**
	 * @return bool
	 */
	function fundor_is_header_builder() {
		global $osf_header;
...
if ( ! function_exists( 'fundor_the_header_builder' ) ) {
	/**
	 * @return void
	 */
	function fundor_the_header_builder() {
		echo fundor_get_header_builder_html();
	}
}

if ( ! function_exists( 'fundor_get_header_builder_html' ) ) {

	/**
	 * @return string
	 */
	function fundor_get_header_builder_html() {
...
if ( ! function_exists( 'fundor_is_footer_builder' ) ) {
	/**
	 * @return bool
	 */
	function fundor_is_footer_builder() {
		global $osf_footer;
...
if ( ! function_exists( 'fundor_the_footer_builder' ) ) {
	/**
	 * @return void
	 */
	function fundor_the_footer_builder() {
		echo fundor_get_footer_builder_html();
	}
}


if ( ! function_exists( 'fundor_get_footer_builder_html' ) ) {
	function fundor_get_footer_builder_html() {
...
if ( ! function_exists( 'fundor_license_get_option' ) ) {
	function fundor_license_get_option( $key = '', $default = false ) {
...
function fundor_social_share() {
	if ( fundor_is_osf_framework_activated() && get_theme_mod( 'osf_socials' ) ) {
		$template = WP_PLUGIN_DIR . '/fundor-core/templates/socials.php';
...
function fundor_social_heading() {
	return esc_html__( 'SHARE:', 'fundor' );
}

add_filter( 'osf_social_heading', 'fundor_social_heading' );

function fundor_get_post_link($taxonomy = 'category', $post_type = ['any']) {
...
if ( ! function_exists( 'fundor_fnc_related_post' ) ) {
	function fundor_fnc_related_post( $relate_count = 4, $posttype = 'post', $taxonomy = 'category' ) {
        ...
        if ( $related->have_posts() ) {
            echo '<div class="post-related-block">';
            echo '<h3 class="related-heading"><span>' . esc_html__( 'You might also like', 'fundor' ) . '</span></h3>';
...
if ( ! function_exists( 'fundor_get_review_counting' ) ) {
	function fundor_get_review_counting() {
...
if (!function_exists('fundor_post_nav')) {
    /**
     * Prints HTML of next and previous in bottom single post style layout
     */
    function fundor_post_nav()
    {

        $obj = fundor_get_post_link('category', 'post');
                    ...
                    <div class="nav-links-inner prev 1">
                        <a href="<?php the_permalink($prev_ID); ?>" rel="prev">
                            ...
                            <div class="nav-content">
                                <div class="meta-nav"><?php esc_html_e('Previous Post','fundor'); ?></div>
                ...
                <?php if (!empty($next_link)): ?>
                    <div class="nav-links-inner next text-right">
                        <a href="<?php the_permalink($next_ID); ?>" rel="prev">
                            <div class="nav-content">
                                <div class="meta-nav"><?php esc_html_e('Next Post','fundor'); ?></div>
...
add_action('fundor_content_single_after', 'fundor_post_nav', 2);